...
- Download to a temporary sibling file and rename atomically. Refuse an existing output path unless `--force` is explicit. Never derive a local path from a remote filename without sanitizing it. `items download` accepts repeatable per-invocation `--allow-redirect-host HOST` overrides for newly documented HTTPS storage hosts; blocked redirect errors show the destination origin and path but never signed query parameters.
